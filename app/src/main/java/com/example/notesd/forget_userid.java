...
public class forget_userid extends AppCompatActivity {

    Toolbar passToolbar;
    EditText registeredNo, enterOtp;
    Button sendOtp, verifyOtp;
    TextView getId, selectCode;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_forget_userid);

        passToolbar   = findViewById(R.id.passToolbar);
        registeredNo  = findViewById(R.id.registeredNo);
        sendOtp       = findViewById(R.id.sendOtp);
        getId         = findViewById(R.id.getId);
        enterOtp      = findViewById(R.id.enteredOtp);
        ...
        selectCode.setOnClickListener(v -> {
            // Initialize dialog
            dialog = new Dialog(forget_userid.this);
            ...
            //ArrayAdapter<String> adapter = new ArrayAdapter<>(forget_userid.this, android.R.layout.simple_list_item_1, countryWithCodeList);

            // set adapter
            //listView.setAdapter(adapter);
            RecyclerView recyclerView = dialog.findViewById(R.id.recyclerView);
            recyclerView.setLayoutManager(new LinearLayoutManager(forget_userid.this));
            CustomRecyclerAdapter adapter = new CustomRecyclerAdapter(forget_userid.this, countryList);
            recyclerView.setAdapter(adapter);
        ...
        sendOtp.setOnClickListener(v -> {
            ...
            String Number = registeredNo.getText().toString();
            String code = selectCode.getText().toString();
            Number = code + Number;
            String RegNum = pref.getString("ContactNo", "");

            Log.d("Numbers", "register Number :" + RegNum + " Number :" + Number);
            USERID = pref.getString("Username", " ");
            SmsManager smsManager = SmsManager.getDefault();

            if(Number.equals(RegNum)){
                GET_OTP = generateOTP();
                smsManager.sendTextMessage(Number, null, GET_OTP + " is the verification code to reset user id", null, null);
        ...
        verifyOtp.setOnClickListener(v -> {
            ENTER_OTP = enterOtp.getText().toString();

            if(GET_OTP.equals(ENTER_OTP)){
                getId.setText("Your User Id ->" + USERID);
            }
